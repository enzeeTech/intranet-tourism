    ...
    <div className="flex-row">
        <div className="">
            <main className="w-full mt-5 xl:pl-96 max-w-[1580px]">
                ...
                <div className="flex items-center justify-between sm:mr-4 md:mr-8 lg:mr-10">
                        ...
                        <>
                            <DragDropContext onDragEnd={handleDragEnd}>
                                <Droppable droppableId="departments">
                                    {(provided) => (
<table className="w-full table-fixed divide-gray-200" {...provided.droppableProps} ref={provided.innerRef}>
<thead>
        <tr>
            <th className="px-6 py-3 font-bold text-left text-gray-500 text-md w-3.5/5">Name</th>
            <th className="px-6 py-3 font-bold text-right text-gray-500 text-md w-1.5/5">Ordering</th>
        </tr>
</thead>
        ...
        {departments.map((item, index) => (
            <Draggable key={item.id} draggableId={String(item.id)} index={index}>
                {(provided) => (
                    ...
                    >
                        <td
                            className="flex items-center px-1 py-4 text-base font-bold text-black whitespace-normal overflow-hidden"
                            style={{ width: '100%' }}
                        ...
                        >
                            <img
                                src={item.imageUrl}
                                alt={item.name}
                                className="w-[60px] sm:w-[40px] md:w-[50px] lg:w-[60px] rounded-md object-cover mr-2 sm:mr-4"
                            />
                            <span className="whitespace-normal text-sm sm:text-base md:text-lg lg:text-xl">
                                {item.name}
                            </span>
                        </td>

                        <td
                            className="px-1 py-4 text-sm font-semibold text-black whitespace-nowrap"
                            style={{ width: '20%' }}
                        >
