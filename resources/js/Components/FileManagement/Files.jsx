...
const FileTable = ({ searchTerm }) => {
  ...
  return (
      <div className="w-full px-4 h-full overflow-y-auto sm:px-0 lg:px-0">
        <div className="flow-root mt-8">
          <div className="overflow-visible">
            <div className="w-full h-[715px] px-8 py-8 rounded-2xl shadow-custom overflow-visible bg-white">
              <table className="w-full bg-white border-separate table-fixed rounded-2xl border-spacing-1">
                  ...
                  {currentItems.map((item, index) => {
                    ...
                    return (
                      <tr key={index}>
                        ...
                        <td className="px-3 py-4 text-sm border-b border-r border-neutral-300 whitespace-nowrap text-neutral-800 text-ellipsis">
                          {item.uploader || 'Unknown'}
                        </td>
                        <td className="px-3 py-4 text-sm border-b border-r border-neutral-300 whitespace-nowrap text-neutral-800 text-ellipsis">
                          {new Date(item.created_at).toLocaleDateString()}
                        </td>
                        <td className="absolute mt-3.5 flex">
                          <PopupContent
                            file={item}
