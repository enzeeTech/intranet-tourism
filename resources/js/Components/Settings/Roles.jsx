...
export default function Roles() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const fetchedRoles = data.data.data.map(role => ({
                    id: role.id,
                    name: roleNameMap[role.id] || role.name,
                }));
                    ...
                    data.data.data
                        ...
                        .map(async (userRole) => {
                            ...
                            if (userResponse.ok) {
                                ...
                                const rolesForUser = Array.isArray(userRole.role_id) ? userRole.role_id : [userRole.role_id]; 
    ...
    const updateRole = async (email, newRole) => {
        try {
            const user = people.find(person => person.email === email);
            let updatedRoles = Array.isArray(user.roles) ? [...user.roles] : [];

            if (newRole.id === 1) {
                // Promote to Super Admin
                if (!updatedRoles.includes(1)) updatedRoles = [...updatedRoles, 1];
                updatedRoles = updatedRoles.filter(roleId => roleId !== 2);
            } else if (newRole.id === 2) {
                // Promote to Admin
                if (!updatedRoles.includes(2)) updatedRoles = [...updatedRoles, 2];
                updatedRoles = updatedRoles.filter(roleId => roleId !== 1);
            } else if (newRole.id === 3) {
                // Demote to User
                updatedRoles = updatedRoles.filter(roleId => roleId !== 1 && roleId !== 2);
            }
                                            ...
                                            <td className="px-3 py-5 text-sm text-gray-500 whitespace-nowrap">
                                                <Menu as="div" className="relative inline-block text-left">
                                                    ...
                                                    <MenuItems className="absolute right-0 z-10 w-full mt-2 origin-top-right bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                                                        {roles
                                                            .filter(role => 
                                                                (person.roles && person.roles.find(roleId => roleId === 2) && role.id === 1) || 
                                                                (person.roles && person.roles.find(roleId => roleId === 1) && role.id === 3) || 
                                                                (role.id === 2 || role.id === 1)
                                                            )
                                                            .map(role => (
