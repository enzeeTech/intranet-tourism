...
function ProfileHeader({ backgroundImage, profileImage, name, status, onEditBanner, rounded }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col self-start px-5 -mt-16 -ml-10 max-md:-ml-3 max-md:mt-5">
        <h1 className="self-end text-3xl font-extrabold text-neutral-800 relative ml-48 max-md:text-xl max-md:ml-5 max-sm:ml-7">{name}</h1>
        <div className="mt-2 text-xs font-semibold text-neutral-800 text-opacity-50 ml-48 max-md:text-sm max-md:ml-5 max-sm:ml-7">{status}</div>
      </div>
    </header>
  );
}
